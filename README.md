...
- `Backtest`
  ...
  The current Bayesian probability-grid contract supersedes the legacy six-row and private-material wording above: each side is independently bounded by `min(10, floor(the relevant chart-boundary distance in complete slots))`. It has no separate half-plot ceiling, so only the actual top or bottom chart edge can crop its corresponding side. The renderer derives the required plot height from the actual quantized horizontal cell size, then publishes that private requirement to the generic Backtest resizer callback. At the resizer minimum, the nearest forecastable curve point within 10% of the plot midpoint retains the intended 10 rows above and below; a truly constrained viewport may reduce rows through the same fit calculation, never by distorting squares, the 1 px logical gap, or integer-bar slots. The probability field remains clipped within the existing plot area and does not change the curve Canvas drawing range.
  ...
  Current authoritative Bayesian Price Field contract (strategy v1.16.0): the preceding release-history prose is retired. The renderer fixes 20 columns, not 36, and renders up to 10 rows independently above and below the price guide. Each square remains exactly mapped to an integer number of trading days and its live-Y-scale price interval; a 20-column field can therefore span more than 20 trading days when integer-day quantization is needed. The preferred width is one quarter of the price plot, while exact time and price mapping, the fixed column count, and the 4 px cell floor take precedence. Cells have a 2 px radius inside a 10 px field radius with 8 px top, bottom, and trailing padding. The production 1 px logical gap applies both between cells and from the vertical guide to the first column. The count on each side is `min(10, floor(relevant chart-boundary capacity))` in complete cell slots; it has no separate half-plot cap. `computeGridMinimumPlotHeight` reuses the actual horizontal lattice cell size to set a private Backtest stage minimum through the generic resizer callback. That lower bound applies on desktop and narrow layouts, protects a real forecastable point within 10% of the plot midpoint at 10 rows above and below, and permits edge-only cropping or a true viewport-fit reduction without aspect-ratio, gap, or integer-bar drift. The private, borderless, shadowless, non-blurred material has `alpha: 0.50`; it neither reads nor changes Settings Frosted Glass tokens. `Bayesian realized-cell score` and `Bayesian lattice coverage` are strictly post-hoc causal-model-lattice diagnostics over horizons 1 through 20. They are not a hit rate for the viewport-sized probability field and never enter signals, fitting, or cache keys. P/E and options remain causal historical factors. P/B, P/S, dividend yield, and market temperature are admitted only with an actual disclosure or safe historical observation timestamp. Capital flow, broker holding, shareholder concentration, fund-holder weight, short interest, and short volume remain exposed for research but are unavailable to the historical model until Longbridge supplies a verified point-in-time availability timestamp and, where needed, a causal aggregation definition. A GPU runtime failure restarts the complete walk-forward calculation on clean NumPy CPU rather than mixing backend results; the presentation records the selected device, numeric precision, and fallback reason.
