...
- `Backtest`
  ...
  The current Bayesian probability-grid contract supersedes the legacy six-row and private-material wording above: each side is independently bounded by `min(10, 50% of the current vertical plot height, its chart boundary)`, so an edge-adjacent guide retains every cell that can fit on the opposite side. The requested 2 px gap is retained when the available slot permits it; cells are square with no radius by default, and each cell maps through the live Y scale to its exact price interval. The matrix container is fully transparent with no outer radius, blur, border, shadow, or background layer; retired tooltip transparency and radius properties are ignored. The probability field is clipped within the existing plot area, so this geometry does not change the curve Canvas drawing range.
