...
- `Backtest`
  ...
  When the Bayesian strategy presentation is available, the history surface places a large Bayesian Price Field detail panel above Metrics or Transactions. It reuses the exact hover cell model and follows the same selected chart index, while retaining the latest valid field after the pointer leaves the overview chart. Its horizontal axis uses the configured Settings date formatter for forecast horizons and limits labels to collision-safe edge-aware ticks; its vertical axis labels the live-Y-scale price range represented by the rendered cells. The detail panel keeps cells square, preserves the 2 px gap and row semantics, and exposes the selected origin through an accessible region and live status.
