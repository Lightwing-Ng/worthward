...
- `Ticker comparison`
  Use the Price metric to review up to 5 tickers on separate charts using their original market-price scales, or the Market cap metric to compare up to 10 historical series. At the registered 900 px sidebar-overlay breakpoint and below, its controls sidebar reuses the Backtest overlay contract: it defaults closed, remembers its own session state, leaves the complete width to results, closes through its backdrop or Escape, and is mutually exclusive with the global navigation sidebar. Mixed-market Price Y axes omit three-letter currency prefixes so their shared numeric gutter remains compact; the shared hover tooltip retains each value's currency. Switching the metric keeps the current sidebar and chart context mounted while the target result hydrates, so the selected segmented pill completes its existing elastic motion without a document reload. A busy metric request can be cancelled by closing its progress dialog and choosing the latest metric intent; invalid ticker sets are rejected before the metric state changes, and a Price switch with more than 5 selected tickers stays in Market cap mode with an explicit validation prompt instead of silently discarding selections. Comparison workspace links also retain their per-view remembered query during the brief page-script loading gap, so a fast click cannot fall back to a blank default workspace. The Price-only `Show chips` control derives an estimated cost distribution from the selected historical OHLCV range, distributes each candle's volume across its low-to-high interval, and renders the result in a cached right-side Canvas profile that shares the price scale. Hovering the price plot truncates each ticker's OHLCV at the shared crosshair timestamp and redraws every profile as the cumulative estimate from the visible range start through that timestamp. Snapshot bins retain the full-range price domain to prevent vertical drift; each subplot prepares cumulative bin prefixes once, and one animation-frame commit owns tooltip content, layout measurement, snapshot selection, and chart redraw while unchanged hover indexes short-circuit. Leaving the chart restores the complete selected-range profile. The profile reports estimated POC, weighted average cost, profit ratio, and central 70% and 90% cost ranges. When a single ticker changes, same-period and same-date successful chip profiles for unchanged tickers are reused; transient ticker errors are excluded from the LRU cache and receive bounded exponential retries. A ticker whose selected-range local OHLCV is sufficiently populated counts as complete and is excluded from fallback requests, preventing an unsupported provider market from producing a false warning for an already rendered profile. The Longbridge request is limited to missing profiles plus one selected anchor ticker when its two-ticker API minimum requires one, and loading is scoped to the missing subplots. When a legacy local history file has no Volume column, every request with valid date bounds, including a one-day exact range, uses range-bounded Longbridge daily OHLCV sequentially and retries the documented one-second rate limit. Current `trade-stats` remains only an unbounded recent-price-level fallback and is never substituted for a historical date-bounded request. Market-cap history uses cached prices and point-in-time yfinance shares with SEC company-facts, filing-level XBRL, and Form N-PORT fallbacks. Non-US quote currencies use same-date daily Yahoo FX closes for USD conversion. Longbridge is optional and can cross-check or replace only the latest trading-day point. The canonical route is `/workspaces/prices`; `?metric=market-cap` selects Market cap, while `/workspaces/market-caps` remains a compatibility redirect.
- `Portfolio`
  Build weighted portfolios and inspect allocation plus aggregate return.
- `Backtest`
  Run any discovered strategy with configurable capital, interval, dividends, and strategy-owned parameters. Every strategy with private parameters exposes them through the shared `Tune strategy parameters` control, which starts pressed and its panel open immediately below Strategy. The complete desktop controls surface scrolls vertically as one logical sequence. At the registered 900 px sidebar-overlay breakpoint and below, Backtest controls leave the results grid and become a default-collapsed, session-remembered left overlay; Performance retains the complete workspace width. The dedicated 44 px parameter button, outside-click backdrop, Escape handling, and mutual exclusion with the global navigation sidebar preserve touch and keyboard access without two overlays colliding. The default-off `Allow algorithmic stop-loss exits` switch prevents a strategy sell or cover signal from closing below its entry price; enabling it permits loss-making algorithmic exits and is not a separate fixed-price stop. This gate compares execution price with entry price only; dividends and total return do not change the decision. For a backtest that follows every Bayesian sell intent regardless of price loss, pass `stop_loss=1` explicitly. Select `Bayesian Price Field` or `LSTM Price Field` for the daily Longbridge CLI probability model; both default to the `NVDA` research ticker and share one probability-grid renderer. LSTM-only controls are namespaced and do not enter Bayesian cache keys. Select `Cycle of Price Action` for a rules-based interpretation of Oliver Kell's 10/20 EMA stage framework (Reversal Extension, Wedge Pop, EMA Crossback, Base n Break, Exhaustion Extension, Wedge Drop): Wedge Pop, EMA Crossback, and Base n Break emit entry intent only when the unchanged Bayesian Price Field rise probability reaches `Field Confirmation`; Wedge Drop, an optional Exhaustion Extension take-profit, or a rise probability at or below `Field Exit Probability` emit exit intent. Stages are causal daily events on the warm-up-inclusive history, reuse the same probability-grid renderer, and are exposed per bar under the presentation's `price_action_cycle` field. Cycle stages do not enter the independent Bayesian probability model, so changing stages alone does not change its forecast. The round `Tune strategy parameters` control opens strategy-declared collapses for model parameters, shared market factors, and optional private actions. LSTM supplies a training action slot with one right-aligned Start/Stop button and a plain history heading; individual history rows have mutually exclusive details, not nested arrow collapses. Capital and return/exit/detail switches belong to the separate `Backtest parameters` collapse. On a strategy switch, that common collapse starts closed when all of its switches are off and stays open when at least one switch is on. All of these groups reuse the `Collapse` component in Style tokens; the strategy selector remains dedicated to strategy choices. The actions are CSRF-protected and the runner state is isolated from market and investment stores. Historical legacy renderer note (not the current contract): the old implementation added a price-anchored crosshair, a stable-width 36-column probability field, six rows per side, and the legacy radii and 4 px cell floor described in the archived release prose below. The current implementation is defined by the authoritative contract in the next paragraph.
  ...
  The current Bayesian probability-grid contract supersedes the legacy six-row and private-material wording above: the reusable display lattice fixes 20 horizontal columns and 12 strategy-owned rows on each side of the signal-close anchor. Each hover side is independently bounded by `min(12, floor(50% of the current plot height in complete slots), floor(the relevant chart-boundary distance in complete slots))`. The half-plot cap prevents an edge-adjacent guide from consuming the whole hover field, while the boundary cap preserves strict clipping. The `Price Field` detail panel uses a separate complete-row geometry, so it renders all 12 rows above and below the selected guide even when the floating hover field is edge-capped; its 20-by-24 grid is then scaled to fit the detail viewport. The renderer derives the required plot height from the actual quantized horizontal cell size, then publishes that private requirement to the generic Backtest resizer callback. At the overview-priority resizer minimum used by `Metrics` and `Transactions`, the nearest forecastable curve point within 10% of the plot midpoint retains the intended 12 rows above and below. While the contained `Price Field` view is active, the detail minimum takes priority and a constrained overview may reduce floating rows through the same fit calculation, never by distorting squares, the 2 px logical gap, or integer-bar slots. The probability field remains clipped within the existing plot area and does not change the curve Canvas drawing range. The 24 display rows change presentation resolution only; the 20 forecast horizons and viewport-independent CRPS scoring contract remain unchanged.

  When a Price Field strategy presentation is available, the history surface adds a third `Price Field` option between `Metrics` and `Transactions`; selecting it preserves both existing views and opens a large Price Field detail panel. It follows the same selected chart index as the hover field, but rebuilds the complete strategy-owned row lattice so edge-capped hover rows are not lost; the detail cells then scale to fit their own viewport. The detail plot stretches across the complete content width assigned by its panel instead of deriving a narrower inline size from available height. Its container-responsive minimum preserves the square 20-by-24 lattice, and the shared vertical splitter gives that minimum priority while `Price Field` is active; Metrics and Transactions retain the overview-priority endpoint. Direct-horizon neural models give this contained detail view an independent, anchor-centered log-price domain derived from the learned horizons' 95% Gaussian log-return envelope and bounded recent-history context. Its 24 rows have equal log-return width, the anchor is the exact middle boundary between the 12 higher-price and 12 lower-price rows, and every cell retains its true positive price bounds. This avoids wasting the lower half of the lattice when a lognormal upper tail spans more dollars than its lower tail, without altering model moments, probabilities, signals, or scores; the overview hover field remains mapped to the live Y scale. The panel retains the latest valid field after the pointer leaves the overview chart. Its status discloses the log-price scale, its horizontal axis uses the configured Settings date formatter for forecast horizons and limits labels to collision-safe edge-aware ticks, and its vertical axis labels the same transformed price range used by cells and observed paths. The detail panel keeps cells square and preserves row semantics. It requests the shared 2 px logical gap, but a genuinely compressed detail viewport reduces the horizontal and vertical gaps together, never below a positive value or at the cost of erasing the cells. The plot and panel remain internally overflow-free, and the responsive result shell does not acquire document-level scrollbars. The panel exposes the selected origin through an accessible region and live status.

  The current Bayesian model targets the executable `Open[t+1] -> Open[t+2]` log return from information available at the close of `t`; the immediately preceding factor row is excluded until both required opens are observable. Multi-column probabilities evolve through a causal AR(1) return state fitted independently at every walk-forward origin, so the renderer no longer freezes one daily conditional mean across the complete horizon. The signal-close remains the display anchor for the absolute-price grid; trading probabilities remain next-open-to-following-open. `Prior Strength` is a percentage of standardized sample information, and enabled factors enter only after positive expanding-window log-score evidence net of a complexity penalty. For every Price Field model, the UI and Markdown export now report the equal-weighted mean of horizon-specific CRPS skill for the standardized close-anchored 1–20 day distribution versus a causal zero-drift volatility baseline. The headline is withheld unless all 20 horizons and every causally eligible forecast pair are scored, preventing incomplete models from improving the displayed result by omitting hard cases. A companion card reports valid-versus-eligible forecast coverage, realized coverage of the central 80% interval, its valid-pair-weighted mean price span relative to each forecast median, and the 1, 5, 10, and 20 day skill profile. Hidden pre-range history supplies causal context, while every scored outcome remains inside the visible backtest range. Overlapping origins and horizons are not independent observations. The legacy next-open direction hit rate and Brier probability score remain machine-readable compatibility diagnostics but are no longer the primary human metrics. Research factors still require a verified historical availability timestamp, and report-period-only fields are discarded to prevent point-in-time leakage.

  The Backtest-wide `Show trade details` switch defaults off for every strategy, including DCA. Turning it off hides the trade markers and equity comparison subplot, gives the price subplot the released space while retaining its time axis, and disables the `Transactions` history option in favor of `Metrics`; it is a display preference and never changes strategy computation or the result cache. The history surface remembers the user's most recent `Metrics`, `Price field`, or `Transactions` choice in browser-local storage and restores it after a strategy change or reload whenever that view is available. An explicit Price Field training run still opens its requested result view first.
  Every choice control in every Backtest strategy parameter panel shares one compact menu contract: the trigger padding, label overflow behavior, page-level menu positioning, and option styling all reuse Leveraged Rotation `Return window`.
  ...
  The rendered cell-size target is derived from the trailing three-month data density on the current chart. Other available ranges reuse that target when the integer trading-day lattice permits it; when exact matching is impossible, the renderer chooses the smallest complete slot that does not undershoot the three-month target. For direct-horizon neural models, this integer-day quantization controls only legible overview spacing: the 20 columns still represent learned horizons 1 through 20 in both hover and detail. Autoregressive Price Fields retain the quantized multi-day horizon stride. Chart boundaries remain authoritative for hover, while the detail panel's separate complete-row geometry is authoritative for its own contained grid; the 20-column density, 4 px floor, and existing strategy row limits remain shared.
...
- `Grid Trading`
  Select Grid Trading directly from Backtest. Its private parameter panel is generated from `strategies/algorithms/strategy_grid_trading.py` and opens through the shared tune control with Current holding, Shares per grid trade, Minimum holding, Maximum holding, Rise %, and Fall %. Each buy or sell is capped at the configured share quantity; its automatic default is the whole-share result of Initial cash divided by ten times the initial price. Current holding represents shares already owned when the simulation starts; Initial cash remains available for later grid purchases, while the first marked value of those shares and the cash together define starting equity and the all-in benchmark. The legacy `/workspaces/grid-trading` URL redirects here with Grid Trading preselected.
- `Trade`
