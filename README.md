...
- `Backtest`
  ...
  When a Price Field strategy presentation is available, the history surface adds a third `Price Field` option between `Metrics` and `Transactions`; selecting it preserves both existing views and opens a large Price Field detail panel. It follows the same selected chart index as the hover field, but rebuilds the complete strategy-owned row lattice so edge-capped hover rows are not lost; the detail cells then scale to fit their own viewport. Direct-horizon neural models give this contained detail view an independent symmetric price domain derived from the selected anchor, the learned horizons' 99% Gaussian envelope, and bounded recent-history context. This preserves vertical forecast resolution on multi-year charts without altering learned probabilities; the overview hover field remains mapped to the live Y scale. The panel retains the latest valid field after the pointer leaves the overview chart. Its horizontal axis uses the configured Settings date formatter for forecast horizons and limits labels to collision-safe edge-aware ticks; its vertical axis labels the adaptive detail price range represented by the rendered cells. The detail panel keeps cells square, preserves the 2 px gap and row semantics, exposes a bar-gradient lower-to-higher-price legend, and exposes the selected origin through an accessible region and live status.
