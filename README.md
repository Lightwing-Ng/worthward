...
- `Backtest`
  ...
  Its Metrics card keeps the compatibility label `Bayesian field hit rate (weighted)` while exposing the precise `probability_weighted_score_pct` and `event_hit_rate_pct` values in the strategy presentation. The former is the probability mass of the realized model-lattice cell; the latter is the percentage of later closes inside that finite lattice. Both are post-hoc diagnostics and never model inputs. Research factors are accepted only when Longbridge supplies an availability or disclosure timestamp; report-period-only fields are discarded to prevent point-in-time leakage.
