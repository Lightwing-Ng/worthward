...
- `tests/e2e/backtest-strategy-params-memory.spec.mjs`: the Backtest strategy-parameter memory regression verifies browser-local persistence across reloads, strategy-scoped values for Grid Trading and DCA, and explicit URL parameters taking precedence without changing the remembered value. Its Leveraged Rotation case reproduces the annotated 1,023 by 1,404 layout, verifies dynamic current-ticker labels, two-decimal percentage controls, independently movable allocation boundaries, token-correct primary/leveraged/cash segments, integer-share-backed cash, and collision-free allocation labels after compact resizing.
