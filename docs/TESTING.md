...
delays a Settings section response and proves that the optimistic skeleton stays
inside the workspace scrollport, the active document is retained, no document View
Transition runs, and the skeleton wrapper is removed when real content arrives.
`tests/e2e/critical_flows/settings-and-navigation.mjs` independently asserts the
scrollport border box and local effect-host boundaries.

On 14 Sep 2026, the prepared-Python complete gate passed Ruff, JavaScript
syntax, 1,818 Python tests with 6 skips and 229 subtests at 75.24% branch
coverage, and 383 JavaScript unit tests. Chromium exercised all 419 cases: 417
passed in the complete batch, while 2 unrelated Backtest cases failed. One was
a test-only missing locator declaration and the other was a transient sidebar
geometry result; their exact two-case rerun passed after the declaration was
repaired. Every Settings case passed in the complete batch. This is complete
suite coverage with an exact recovery rerun, not one uninterrupted green gate.
...
WSL or a Unix host. No native PowerShell full gate is provided.

The gate checks its Python quality dependencies before starting and names the
selected interpreter when dependencies are missing. Set `WORTHWARD_PYTHON`
to a prepared interpreter or install dependencies with the setup wrapper.

A repository-scoped lock protects the complete gate and its coverage reports.
A competing complete run exits `73` before writing reports. Standalone coverage
commands must use separate output paths; the browser stage also acquires its
existing host-wide port lock. `scripts/check_steps.sh` is an internal worker;
always invoke `scripts/check.sh`.

Layout-only LSTM browser tests explicitly request CPU, one training epoch,
...
Historical suite inventory measured on 28 Aug 2026 (not the current count):
...
- `tests/test_investment_import.py` and `tests/test_more_page.py`: IBKR Trade
  ...
  tails for partially overlapping closed report intervals, exact Forex P/L
  detail attachment without a second fee deduction, silent cash-section total
  rows, newer date-only cash boundaries that clear stale intraday timestamps,
...
- `tests/test_price_field_scoring.py`, `tests/test_neural_price_field_scoring.py`, and strategy integration tests: Gaussian CRPS math, causal zero-drift references, equal-horizon aggregation, candidate-equals-reference zero skill, complete 20-horizon and complete eligible-pair headline gates, warmup-inclusive causal context with visible-range outcomes, valid-pair-weighted central intervals, direct-versus-independent distribution parity, summary/export mapping, and the LSTM GA's validation-only CRPS ranking with incomplete folds rejected. Browser regressions additionally require readable diagnostic geometry, keyboard access to a scrollable Metrics region, and unclipped complete and incomplete evidence in the Backtest share card.
...
isolated backend, much less the user's investment store. The HSBC invalid-paste
case keeps the import modal open and proves that the portaled error banner has a
higher computed layer, remains inside the viewport, and owns its center-point
paint hit.
