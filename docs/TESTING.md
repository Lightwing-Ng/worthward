...
- `tests/e2e/lstm-price-field.spec.mjs` and `tests/e2e/shared-backtest-controls.spec.mjs`: LSTM regressions verify that Compute backend is the first training field, action-body padding is 4px inline and zero at the bottom, history entries have zero padding, factor-group titles report checked counts, and training-only backend/factor drafts reach Start training without a form submit, URL mutation, or current-chart recalculation.
