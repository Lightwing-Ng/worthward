...
primitives. Its lock combines in-process thread exclusion with a host file lock.
LSTM and Price Field managers reuse those mechanics while keeping their distinct
validation, process launch, stop, archive, and result semantics. They must not be
collapsed into a generic worker lifecycle merely because both persist job files.
The shared module never initializes or migrates market, settings, investment, or
broker-evidence stores.

Other narrow reuse seams follow the same rule: runtime network diagnostics own
URL credential and secret-query redaction; the Longbridge SDK adapter owns config
construction across supported package versions; storage owns one row-to-record
...
strategy modules; `create_app()` is the composition boundary. Executable AST
contracts enforce `core/models -> infrastructure -> services -> web` dependency
direction, restrict split route modules to Flask plus `WebRuntime`, and require
the runtime dataclass, builder keywords, and route consumers to remain exactly
aligned.
...
specimen. One binding synchronizes labels and track segments on input and owns
measured label collision, edge alignment, and close-handle geometry. The catalog
uses illustrative capital of 10,000 and displays proportional cash; production
retains its existing whole-share purchases and residual-cash calculation. The
catalog adapter does not submit forms or persist its demonstration values.
...
job and passes the form state to the runner as `--selected-params`, with an
explicit direction diagnostic objective. That selected-configuration path
bypasses GA population generation, crossover, and mutation. Request identity,
launch metadata, and the market snapshot retain the interval. This trainer
supports only `1d`: missing or unsupported frequencies fail before launch,
never silently switching `1m` to daily bars. The market loader receives the
selected configuration and date bounds; provider source metadata and actual
OHLCV observations remain intact. Missing provider data fails closed.
Unavailable historical factor observations are never fabricated; the shared
model's factor-availability rules still apply.

Web-selected exact training allocates at least 180 seconds of completed
...
The standalone CLI owns genetic mutation tuning. Without `--selected-params`,
`scripts/lstm_ga_tune.py` defaults to a 36,000-second scheduling budget and the
validation-only, equal-horizon 1–20 day CRPS skill objective against the causal
baseline. `--base-params` may supply a starting configuration while uniform
crossover and mutation vary eligible hyperparameters and available factors. All
...
measured holdout accuracy badge. A genetic-mode result instead exposes its
mean validation CRPS skill, objective, budget, and submitted baseline when those
fields exist. Clicking a completed exact run restores its submitted configuration;
clicking a completed robust GA result restores the seed-42 member of the selected
cohort. Both restore ticker, original period, exact scored data dates, interval,
capital, return/exit controls, and all private parameters. The requested window remains
...
green token only. Terminal jobs have no bar. Rows show ticker, the score appropriate
to their exact-training or genetic mode, and right-aligned
...
Protocol version 3 prevents the updated UI from starting jobs through stale
...
panel derives that budget from the owning container's inline size, while the
detail plot itself stretches across the complete panel content width. It never
publishes a narrower inline width from the available height. While `Price Field`
is active, the shared splitter makes this history minimum authoritative at its
maximum overview endpoint; `Metrics` and `Transactions` continue to prioritize
the overview minimum. This prevents splitter movement from reducing the detail
lattice below the resolution already imposed by its available width. The plot,
panel, and responsive result shell retain zero horizontal and vertical overflow.
...
and Transactions share the same resizer endpoint. At `900px` and below, the Backtest
primary title rail reserves both left controls and centers its heading row on the same
vertical axis as the sidebar, Backtest-parameter, and theme buttons. The Backtest
result title remains below that rail in compact flow.
...
Return comparison and Ticker comparison share ticker, relative-range, exact-date, and per-view session-memory infrastructure. Ticker comparison defaults to Price and retains the existing 5-ticker price-subplot contract; its `metric=market-cap` mode reuses the comparison controls and range workflow while allowing 10 tickers. At 900 px and below, the Ticker comparison controls use the same workspace-controls overlay controller, responsive shell, transparent dismissal layer, inert closed state, Escape focus return, and global-sidebar mutual exclusion as Backtest. The comparison and Backtest overlays keep separate session-storage keys, so opening one never changes the remembered state of the other. Market-cap history is derived from authoritative cached prices and point-in-time Yahoo-reported shares outstanding, with SEC company facts and filing-level XBRL as rate-limit fallbacks. Funds without company-facts shares use SEC Form N-PORT net assets. For the latest trading day, Longbridge `mktcap` and `last_done` provide an independent implied-share cross-check and the preferred current point. Non-US market caps are converted at the same-date daily Yahoo FX close into the immutable USD base currency; the comparison axis remains America/New_York. The service records matched, review, or diverged status after normalizing comparable providers to the same price; missing pre-disclosure periods remain unknown, and current Longbridge shares are never backfilled into older dates.
