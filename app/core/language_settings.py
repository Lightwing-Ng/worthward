...
SETTINGS_TRANSLATION_ROWS = _build_translation_rows(
    (
        ...
        ("Authorize through the installed Longbridge CLI in a separate browser window. Antigravity does not receive, display, or store the authorization code or OAuth token. Your existing terminal CLI profile is used automatically.", "在獨立瀏覽器視窗中透過已安裝的 Longbridge CLI 授權。Antigravity 不會接收、顯示或儲存授權碼或 OAuth token，並會自動使用現有的終端機 CLI 設定檔。", "在独立浏览器窗口中通过已安装的 Longbridge CLI 授权。Antigravity 不会接收、显示或存储授权码或 OAuth token，并会自动使用现有的终端 CLI 配置文件。"),
