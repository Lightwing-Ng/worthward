...
`investment_import_*.py` domain modules, and the facade re-exports them.
...
owns the implementation. `tests/test_investment_import_compat_boundary.py`
...
PATCHABLE_SEAMS: dict[str, str] = {
    "_load_local_private_investment_evidence": (
        "app.services.investment_import_basics"
    ),
    "_extract_statement_pdf_text": "app.services.investment_import_usmart_tiger",
    ...
    "_has_same_hsbc_cash_source_row": (
        "app.services.investment_import_payload_summaries"
    ),
    "build_investment_internal_transfer_binding_index": (
        "app.services.investment_import_bindings"
    ),
}
