...
- Changed: Public bundle, loader, and cache names use Price Field terminology;
  the former Bayesian names remain compatibility aliases at the end of this
  module.
...
_BUNDLE_CACHE: OrderedDict[
    tuple[object, ...],
    tuple[float, "PriceFieldMarketFactorBundle"],
] = OrderedDict()
_BUNDLE_IN_FLIGHT: dict[
    tuple[object, ...],
    Future["PriceFieldMarketFactorBundle"],
] = {}
...
@dataclass(frozen=True)
class PriceFieldMarketFactorBundle:
    symbol: str
    start: date
    end: date
...
def build_local_price_field_factor_bundle(
        symbol: str,
    ...
    local-first process must still be able to render a Price Field backtest when
    ...
    from app.services.market.market_data import fetch_history
    ...
    return PriceFieldMarketFactorBundle(
        symbol=normalized_symbol,
...
def clear_price_field_factor_cache() -> None:
...
def fetch_price_field_factor_bundle(
        symbol: str,
        start: date | datetime | str,
        end: date | datetime | str,
        *,
...
# Backward-compatible names for callers that still use the historical provider
# module API. The canonical implementation and names above are model-neutral.
BayesianFactorBundle = PriceFieldMarketFactorBundle
build_local_bayesian_factor_bundle = build_local_price_field_factor_bundle
clear_bayesian_factor_cache = clear_price_field_factor_cache
fetch_bayesian_factor_bundle = fetch_price_field_factor_bundle
