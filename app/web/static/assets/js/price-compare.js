...
(() => {
	...
	const priceCompareRuntime = window.WORTHWARD_PRICE_COMPARE_RUNTIME;
	if (!priceCompareRuntime) return;
	const {
	...
	} = priceCompareRuntime.createSessionAxis({
		bootstrap,
		state,
		chartAxis,
		yAxisMinWidth: Y_AXIS_MIN_WIDTH,
	});
	...
	const {
		chipRevealMotion,
		getChipRevealLogoOrigins,
		prepareChipRevealMotion,
		settleChipRevealMotion,
		startChipRevealMotion,
	} = priceCompareRuntime.createChipRevealController({
		isChipsEnabled,
		priceCharts,
	});
		...
		document.querySelectorAll("[data-price-subplot-canvas]").forEach((canvas) => {
			...
			const closingLogoPlugin = {
				...
				afterDatasetsDraw(chart) {
					...
					const origin = chipsEnabled ? getChipRevealLogoOrigins().get(index) : null;
