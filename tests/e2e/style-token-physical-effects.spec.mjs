...
const targetSelectors = [
    '[data-style-token-card="circular-icon-button"] .style-token-demo',
    '[data-style-token-card="primary-button"] .style-token-demo > button',
    '[data-style-token-card="pagination"] .style-token-demo',
    '[data-style-token-card="scrollable-table"] .style-token-demo .scrollable-data-table-shell',
    '[data-style-token-card="portfolio-donut-orbit"] .style-token-demo',
    '[data-style-token-card="tooltip"] .style-token-demo',
];
...
test('keeps Style tokens physical effects outside the workspace clip', async ({page}) => {
    ...
    await expect.poll(async () => Promise.all(
        targetSelectors.map((selector) => page.locator(selector).count()),
    )).toEqual([1, 1, 1, 1, 1, 1]);
    ...
    const layout = await page.evaluate(() => {
        ...
        const demo = document.querySelector('[data-style-token-card="tooltip"] .style-token-demo');
