...
from tests.support.app_test_utils import read_app_bundle
from tests.support.css_test_utils import read_css_bundle
from tests.support.runtime_test_utils import read_runtime_bundle
from tests.support.template_test_utils import read_template_bundle
...
def _read(path: Path) -> str:
    ...
    if path.name == "chart-controller.js":
        mount_helper = path.with_name("chart-controller-mount.js")
        entry_source = path.read_text(encoding="utf-8")
        # Preserve the legacy semantic-source assertions across the explicit
        # mount context introduced by the physical file split.
        mount_source = mount_helper.read_text(encoding="utf-8").replace(
            "continuation.", ""
        )
        return mount_source + entry_source.replace("mount.", "")
...
def test_compare_share_and_date_rows_use_the_same_summary_grid() -> None:
    ...
    summary_branch = share_script[
        share_script.index('dataset.sharePlacement === "summary-panel"') :
    ]
    summary_branch = summary_branch[
        : summary_branch.index('dataset.sharePlacement === "summary-heading"')
    ]
...
def test_portfolio_uses_a_single_line_short_title() -> None:
    ...
    workspace = _read(ASSET_ROOT / "css/views/workspace.css")
    assert (
        ".portfolio-workspace .workspace-mode-results-stack > .chart-surface {"
        in workspace
    )
...
def test_broker_feedback_uses_the_copy_column_and_own_layout_row() -> None:
    ...
    version = _css_code_version(ASSET_ROOT / "css/views/settings.css").removeprefix("v")
    assert f'@import url("./views/settings.css?v={version}");' in app_css
    assert (
        f'@import url("./foundation/tokens.css?v={_css_code_version(ASSET_ROOT / "css/foundation/tokens.css").removeprefix("v")}");'
        in app_css
    )
    assert (
        f'@import url("./components/forms.css?v={_css_code_version(ASSET_ROOT / "css/components/forms.css").removeprefix("v")}");'
        in app_css
    )
    investment_version = _css_code_version(
        ASSET_ROOT / "css/views/investment.css"
    ).removeprefix("v")
    assert f'@import url("./views/investment.css?v={investment_version}");' in app_css
...
def test_bayesian_compute_backend_value_is_centered_in_its_own_column() -> None:
    ...
    assert (
        '.trade-strategy-param[data-strategy-param-key="compute_backend"] .trade-strategy-param-select-shell {'
        in trade_css
    )
    assert "justify-self: center;" in trade_css
    assert "width: min(100%, 160px);" in trade_css
    assert (
        '.trade-strategy-param[data-strategy-param-key="compute_backend"] .trade-strategy-trigger {'
        in trade_css
    )
    assert "justify-content: center;" in trade_css
    assert "--compute-backend-trigger-label-offset" in trade_css
    assert (
        "transform: translateX(var(--compute-backend-trigger-label-offset));"
        in trade_css
    )
    trade_version = _css_code_version(ASSET_ROOT / "css/views/trade.css").removeprefix(
        "v"
    )
    assert f'@import url("./views/trade.css?v={trade_version}");' in app_css
...
def test_backtest_transaction_columns_fill_each_responsive_table_width() -> None:
    ...
    assert (
        ".backtest-history-table .trade-transactions-number {" in legacy_width_override
    )
...
def test_backtest_shared_controls_use_the_sampled_geometry_contract() -> None:
    ...
    controls_block = forms_css[
        controls_start : forms_css.index('input[type="number"]', controls_start)
    ]
    ...
    parameter_rule = forms_css[
        parameter_start : forms_css.index("\n}", parameter_start)
    ]
    assert "padding-top: 0;" in parameter_rule
    assert (
        "min-height: calc(var(--strategy-tune-panel-row-height) + 1px);"
        in parameter_rule
    )
...
def test_backtest_boolean_switches_share_the_plain_switch_row_contract() -> None:
    ...
    assert (
        'row_class="switch-row switch-row--plain trade-strategy-boolean-row"'
        in strategy_template
    )
    assert (
        ".trade-strategy-boolean-row .switch-label > span:not(.field-tooltip)"
        not in forms_css
    )
...
def test_backtest_section_resizer_uses_compact_handle_contract() -> None:
    ...
    assert (
        "--investment-section-resizer-size: var(--backtest-section-resizer-size);"
        in trade_css
    )
...
def test_backtest_probability_scroll_delegates_paint_to_the_native_browser() -> None:
    trade_css = _read(ASSET_ROOT / "css/views/trade.css")
    backtest_script = _read(ASSET_ROOT / "js/backtest.js") + _read(
        ASSET_ROOT / "js/backtest/chart-controller.js"
    )
    backtest_template = _read(TEMPLATE_ROOT / "backtest.html")
    pending_app = _read(ASSET_ROOT / "js/app.js")
    stack_contract = trade_css[
        trade_css.index(".trade-chart-stack {") : trade_css.index(
            ".backtest-probability-scroll-spacer {"
        )
    ]
    scroll_contract = trade_css[
        trade_css.index(".backtest-probability-scrollport {") : trade_css.index(
            ".backtest-probability-scrollport-spacer {",
    ...
    for markup in (backtest_template, pending_app):
        assert (
            'class="backtest-section-resizer-slot" data-backtest-section-resizer-slot'
            in markup
        )
...
def test_bayesian_backtest_routes_dynamic_grid_minimum_through_shared_resizer() -> None:
    ...
    backtest_asset_order = (
        "assets/js/backtest/probability-grid.js",
        "assets/js/backtest/detail-chart.js",
        "assets/js/backtest/chart-controller-mount.js",
        "assets/js/backtest/chart-controller.js",
        "assets/js/backtest.js",
    )
    assert [base_template.index(path) for path in backtest_asset_order] == sorted(
        base_template.index(path) for path in backtest_asset_order
    )


def test_bayesian_history_detail_preserves_hover_and_complete_geometry() -> None:
    backtest_template = _read(TEMPLATE_ROOT / "backtest.html")
    probability_field_partial = _read(
        TEMPLATE_ROOT / "_backtest_probability_field.html"
    )
    backtest_script = _read(ASSET_ROOT / "js/backtest.js") + _read(
        ASSET_ROOT / "js/backtest/chart-controller.js"
    )
    ...
    bayesian_strategy = _read(
        PROJECT_ROOT / "strategies/algorithms/strategy_bayesian_price_field.py"
    )
    ...
    assert (
        "PROBABILITY_GRID_RENDERER" in lstm_strategy
        or "probability-grid-v1" in lstm_strategy
    )
    ...
    assert (
        'aria-label="Normalized directional shares of represented probability mass"'
        in probability_field_partial
    )
    assert (
    ...
    assert backtest_template.index('"value": "metrics"') < backtest_template.index(
        '"value": "probability"'
    )
    assert backtest_template.index('"value": "probability"') < backtest_template.index(
        '"value": "transactions"'
    )
    assert backtest_template.index(
        'data-backtest-history-view-panel="metrics"'
    ) < backtest_template.index('{% include "_backtest_probability_field.html" %}')
    assert backtest_template.index(
        '{% include "_backtest_probability_field.html" %}'
    ) < backtest_template.index('data-backtest-history-view-panel="transactions"')
    for fragment in (
        "pointRadius: 0,\n\t\t\t\t\t\tpointHoverRadius: 0,",
...
def test_settings_layout_dimensions_are_canonical_and_color_groups_follow_the_intro() -> (
    None
):
    tokens = _read(ASSET_ROOT / "css/foundation/tokens.css")
    settings_css = _read(ASSET_ROOT / "css/views/settings.css")
    ...
    intro_position = settings_template.index(
        'class="settings-color-token-intro settings-card"'
    )
    sidebar_position = settings_template.index('class="settings-color-token-sidebar"')
    group_position = settings_template.index(
        'class="settings-color-token-group settings-card"'
    )
...
def test_period_controls_use_the_shared_dropdown_width_token() -> None:
    ...
    assert (
        ".ticker-controls .field.range-panel > .backtest-shared-select-row {"
        in forms_css
    )
...
def test_shared_segmented_controls_shrink_wrap_without_disabling_true_overflow_tracks() -> (
    None
):
    forms_css = _read(ASSET_ROOT / "css/components/forms.css")
    investment_css = _read(ASSET_ROOT / "css/views/investment.css")
    ...
    backtest_interval_rule = forms_css[
        forms_css.index(
            ".trade-controls .backtest-interval-field > .backtest-interval-segmented {"
        ) : forms_css.index(".trade-controls .range-mode-field > .range-mode-shell {")
    ]
    range_mode_rule = forms_css[
        forms_css.index(
            ".trade-controls .range-mode-field > .range-mode-shell {"
        ) : forms_css.index(".trade-controls .range-mode-field {")
    ]
    peek_rule = forms_css[
        forms_css.index(
            '.segmented-control[data-segmented-overflow-mode="peek"] {'
        ) : forms_css.index(
            '.segmented-control[data-segmented-overflow-mode="peek"][data-segmented-overflow="1"] {'
        )
    ]
...
def test_strategy_parameters_reveal_downward_without_crossing_the_strategy_row() -> (
    None
):
    forms_css = _read(ASSET_ROOT / "css/components/forms.css")

    popover_rule = forms_css[
        forms_css.index(".trade-strategy-params-popover {") : forms_css.index(
            ".trade-controls:has(.trade-strategy-field.is-open)"
        )
    ]
    ...
    animation = forms_css[
        forms_css.index("@keyframes strategy-params-flow-in {") : forms_css.index(
...
def test_shared_segmented_controls_keep_visible_disabled_options_in_their_slots() -> (
    None
):
    forms_css = _read(ASSET_ROOT / "css/components/forms.css")
    app_js = _read(ASSET_ROOT / "js/app.js")
    ...
    assert (
        '[data-backtest-history-transactions-option][aria-disabled="true"]'
        not in trade_css
    )


def test_simplified_chinese_label_uses_the_ascii_parenthesis_contract() -> None:
    language_settings = _read(PROJECT_ROOT / "app/core/preferences/language.py")
    runtime = read_runtime_bundle()
    template = _read(TEMPLATE_ROOT / "settings.html")
...
def test_production_templates_publish_the_shared_layout_role_registry() -> None:
    ...
    for template_name in (
        ...
        assert 'data-layout-role="result-container"' in _read(
    ...
    assert 'data-layout-role="secondary-heading"' in _read(
        TEMPLATE_ROOT / "investment.html"
    )
    assert 'data-layout-role="pagination"' in _read(
        TEMPLATE_ROOT / "_dca_backtest_table.html"
    )
    assert 'data-layout-role="content-scrollport"' in _read(
        TEMPLATE_ROOT / "settings.html"
    )
...
def test_backtest_title_rails_reuse_shared_control_alignment() -> None:
    ...
    assert (
        '<article class="report-card workspace-article-card workspace-summary-card workspace-mode-title-card" data-layout-role="title-rail">'
        in template
    )
    assert (
        '<article class="report-card workspace-article-card workspace-summary-card" data-layout-role="result-title-rail">'
        in template
    )
    assert ".workspace-mode-main.backtest-workspace-main {" not in workspace
    assert (
        "height: calc(100% + var(--workspace-mode-result-heading-lift));" in workspace
    )
    assert (
...
def test_effect_hosts_and_scrollports_have_explicit_overflow_ownership() -> None:
    ...
    trade_stack = trade_css[
        trade_stack_start : trade_css.index("\n}", trade_stack_start)
    ]
    assert "overflow: hidden;" in trade_stack
    workspace_version = _css_code_version(
        ASSET_ROOT / "css/views/workspace.css"
    ).removeprefix("v")
