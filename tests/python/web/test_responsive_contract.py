"""Tests for the unified responsive breakpoint contract. Code version: v0.2.10."""
...
def test_coarse_pointer_sidebar_toggle_keeps_its_hit_target_stationary() -> None:
    ...
    for fragment in (
        ...
        "width: var(--circular-icon-button-size);",
        "min-width: var(--circular-icon-button-size);",
        "height: var(--circular-icon-button-size);",
        "min-height: var(--circular-icon-button-size);",
        "touch-action: manipulation;",
        "z-index: var(--layer-sidebar-toggle);",
