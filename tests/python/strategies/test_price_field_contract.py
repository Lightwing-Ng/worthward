"""Shared Price Field contract tests. Code version: v1.6.2."""
...
import strategies.price_field.pipeline as pipeline_module
...
class PriceFieldContractTests(unittest.TestCase):
    ...
    def test_factor_subgroups_are_reusable_without_duplicating_controls(self) -> None:
        from app.web.presentation.strategy_forms import build_strategy_form_fields, build_strategy_form_sections
    ...
    def test_market_factor_provider_uses_a_model_neutral_canonical_module(self) -> None:
        ...
        pipeline_source = (
            root / "strategies/price_field/pipeline.py"
        ).read_text(encoding="utf-8")
        legacy_source = (
            root / "app/services/bayesian_market_factors.py"
        ).read_text(encoding="utf-8")

        self.assertIn("app.services.research.price_field_market_factors", pipeline_source)
