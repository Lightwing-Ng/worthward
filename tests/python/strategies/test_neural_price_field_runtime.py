"""Neural inference pipe progress and process ownership. Code version: v1.0.1."""
...
from strategies.price_field.neural.compute import NeuralForecast, NeuralTrainingCancelled
from strategies.price_field.neural import runtime as runtime
...
def test_pipe_round_trip_forwards_progress_once_and_preserves_missing_predictions():
    ...
    assert launch.call_args.args[0] == [
        "/installed/torch/python", "-B",
        str(project_root / "scripts/neural_price_field_infer.py"),
    ]
    assert launch.call_args.kwargs["cwd"] == project_root
    assert (project_root / "scripts/neural_price_field_infer.py").is_file()
