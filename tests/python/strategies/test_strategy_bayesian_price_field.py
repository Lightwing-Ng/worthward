...
from app.web.presentation.strategy_forms import build_strategy_form_field
...
class BayesianPriceFieldStrategyTests(unittest.TestCase):
    ...
    def test_option_detail_selection_requests_shared_history_without_composite(self) -> None:
        ...
        strategy = BayesianPriceFieldStrategy()
        with patch(
            "app.services.research.price_field_market_factors.fetch_price_field_factor_bundle",
            return_value=bundle,
        ) as fetch_bundle:
    ...
    def test_market_loader_uses_local_store_when_remote_access_is_disabled(self) -> None:
        ...
        with (
            patch(
                "strategies.price_field.pipeline.is_remote_market_access_disabled",
                return_value=True,
            ),
            patch(
                "app.services.research.price_field_market_factors.build_local_price_field_factor_bundle",
                return_value=bundle,
            ) as local_bundle,
            patch(
                "app.services.research.price_field_market_factors.fetch_price_field_factor_bundle",
