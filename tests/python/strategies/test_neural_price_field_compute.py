"""Direct neural distribution causality and backend contracts. Code version: v1.0.1."""
...
from strategies.price_field.neural import compute as compute
...
def test_missing_torch_dispatches_to_supported_runtime_without_recursive_fallback():
    features, closes = _inputs(64)
    with patch.object(compute, "_load_torch", side_effect=RuntimeError("missing torch")), patch.dict("os.environ", {"WORTHWARD_NEURAL_INFERENCE_WORKER": "0"}), patch("strategies.price_field.neural.runtime.infer_in_supported_runtime", return_value="bridged") as bridge:
