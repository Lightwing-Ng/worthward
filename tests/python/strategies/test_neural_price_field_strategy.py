"""Shared neural strategy and causal input contracts. Code version: v1.5.1."""
...
from strategies.price_field.neural.compute import NeuralForecast
from strategies.price_field.neural.registry import NEURAL_ARCHITECTURES, NEURAL_SPECS
...
def test_model_import_does_not_initialize_the_web_application():
    result = subprocess.run(
        [sys.executable, "-B", "-c", "import sys; import strategies.price_field.neural.strategy; assert 'app' not in sys.modules"],
...
def test_each_architecture_emits_direct_horizon_contract_on_cpu(architecture):
    ...
    assert lattice["columns"] == 20
    assert lattice["rows_above"] == 12
    assert lattice["rows_below"] == 12
...
def test_hidden_history_scores_only_visible_direct_neural_origins():
    ...
    with patch(
        "strategies.price_field.neural.strategy.walk_forward_neural_predictions",
