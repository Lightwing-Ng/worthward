...
class LstmComputeTests(unittest.TestCase):
    def test_target_normalization_is_causal_and_affine_equivariant(self):
        ...
        with patch("strategies.price_field.lstm_compute._numpy_origin_prediction", side_effect=predict):
    ...
    def test_vectorized_origin_windows_preserve_scalar_causal_selection(self):
        from strategies.price_field.lstm_compute import _gather_origin_batch
    ...
    def test_backends_receive_identical_causal_standardized_inputs(self):
        ...
        for engine, function in (("numpy", "_numpy_origin_prediction"), ("torch", "_torch_train_and_predict")):
            ...
            with patch(f"strategies.price_field.lstm_compute.{function}", side_effect=predict):
    ...
    def test_minimum_work_continues_optimizer_updates_without_reinitializing(self):
        ...
        with patch("strategies.price_field.lstm_compute.time.perf_counter", side_effect=[0, 1, 1, 2, 2, 3]):
    ...
    def test_durable_gpu_errors_do_not_claim_a_cpu_training_success(self):
        ...
        with patch("strategies.price_field.lstm_compute._torch_train_and_predict", side_effect=RuntimeError("device lost")):
    ...
    def test_cpu_backend_does_not_require_a_gpu_probe(self) -> None:
        with patch("strategies.price_field.lstm_compute.probe_torch_device") as probe:
    ...
    def test_neural_engine_falls_back_without_claiming_npu_use(self) -> None:
        for apple_silicon, reason in (
            ...
            with (
                ...
                patch("strategies.price_field.lstm_compute._apple_silicon", return_value=apple_silicon),
                patch("strategies.price_field.lstm_compute._load_optional_module", return_value=None),
            ):
