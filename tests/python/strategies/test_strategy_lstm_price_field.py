...
def _bundle_from_frame(frame: pd.DataFrame) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        dynamic_pe_history=(),
        option_history=(),
        fingerprint="lstm-warmup-boundary",
        factor_status={
            "ohlcv": "available",
            "pe": "missing",
            "options": "missing",
...
class LSTMPriceFieldStrategyTests(unittest.TestCase):
    ...
    def test_disabled_vap_skips_kernel_and_chip_window_warmup(self):
        from strategies.price_field.pipeline import load_price_field_market_bundle

        with patch("strategies.price_field.pipeline._rolling_volume_at_price_percentile",
        ...
        starts = []
        with patch("strategies.price_field.pipeline.is_remote_market_access_disabled", return_value=True), patch(
                "app.services.research.price_field_market_factors.build_local_price_field_factor_bundle") as loader:
    ...
    def test_strategy_depends_on_shared_pipeline_not_bayesian_strategy(self) -> None:
        source = Path(
            __file__
        ).resolve().parents[3] / "strategies/algorithms/strategy_lstm_price_field.py"
    ...
    def test_presentation_matches_the_shared_probability_grid_contract(self) -> None:
        ...
        grid = presentation["diagnostics"]["grid"]
        ...
        self.assertEqual(
            presentation["diagnostics"]["distribution_metric_kind"],
            "close-anchored-standardized-1-20d-crps-skill",
        )
        self.assertEqual(
            presentation["diagnostics"]["distribution_evaluation_scope"],
            "visible-backtest-range-with-causal-prior-history",
        )
        self.assertIn("80", grid["central_intervals"])

    def test_hidden_history_scores_only_visible_origins(self) -> None:
        ...
        self.assertEqual(
            grid["eligible_pairs"],
            sum(max(len(visible_frame) - horizon, 0) for horizon in range(1, 21)),
        )
        self.assertEqual(grid["valid_pairs"], grid["eligible_pairs"])
        self.assertEqual(diagnostics["distribution_warmup_history_points"], 60)
        self.assertEqual(diagnostics["distribution_visible_origin_points"], 60)
    ...
    def test_gpu_request_reports_cpu_fallback_instead_of_the_request(self) -> None:
        with patch("strategies.price_field.lstm_compute._load_optional_module", return_value=None):
            result = LSTMPriceFieldStrategy().compute_signals(
                _market_frame(72),
