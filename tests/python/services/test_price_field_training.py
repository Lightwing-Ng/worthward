...
from app.services.research import price_field_training as training
...
def test_training_job_paths_resolve_from_the_actual_checkout(tmp_path, pytestconfig):
    checkout_root = pytestconfig.rootpath.resolve()
    assert training.PROJECT_ROOT == checkout_root
    assert (checkout_root / "scripts/price_field_train.py").is_file()
    manager = training.PriceFieldTrainingManager(tmp_path)
    assert manager.workspace_root() == project_compute_workspace_root(
        tmp_path,
        checkout_root,
        "probability-models",
    )
...
def test_exact_worker_trains_real_cpu_model_with_isolated_market_snapshot(tmp_path, monkeypatch, strategy_id):
    ...
    from strategies.price_field.neural import compute as engine
