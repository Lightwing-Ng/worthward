...
def test_runner_and_manager_share_the_project_workspace_root(tmp_path, pytestconfig):
    checkout_root = pytestconfig.rootpath.resolve()
    assert lstm_training.PROJECT_ROOT == checkout_root
    assert ga_runner.PROJECT_ROOT == checkout_root
    assert Path(ga_runner.__file__).resolve() == checkout_root / "scripts/lstm_ga_tune.py"
    manager = lstm_training.LstmTrainingManager(tmp_path)
