...
from app.services.research import price_field_market_factors as factors
...
def test_research_source_manifests_reference_existing_checkout_modules(pytestconfig):
    from scripts import bayesian_crps_ga_job as durable_job
    from scripts import prepare_bayesian_crps_ga_config as preparation

    checkout_root = pytestconfig.rootpath.resolve()
    factor_source = Path(factors.__file__).resolve().relative_to(checkout_root).as_posix()
    assert preparation.PROJECT_ROOT == checkout_root
    assert factor_source in preparation.CRITICAL_SOURCE_PATHS
    assert factor_source in durable_job.PREPARATION_SOURCE_PATHS
    source_paths = (
        set(preparation.CRITICAL_SOURCE_PATHS)
        | durable_job.REQUIRED_RUNTIME_SOURCE_PATHS
        | durable_job.PREPARATION_SOURCE_PATHS
    )
    assert source_paths
    missing_paths = sorted(path for path in source_paths if not (checkout_root / path).is_file())
...
class PriceFieldMarketFactorProviderTests(unittest.TestCase):
    ...
    def test_local_bundle_uses_existing_daily_store_without_provider_factors(self) -> None:
        ...
        with patch("app.services.market.market_data.fetch_history", return_value=frame) as fetch:
    ...
    def test_local_bundle_requires_observed_ohlc_columns(self) -> None:
        ...
        with patch("app.services.market.market_data.fetch_history", return_value=frame):
        ...
        for column in ("Open", "High", "Low", "Close"):
            for invalid in (float("nan"), float("inf"), -float("inf"), pd.NA):
                with self.subTest(column=column, invalid=invalid):
                    ...
                    with patch("app.services.market.market_data.fetch_history", return_value=damaged):
