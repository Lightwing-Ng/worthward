...
class LogoServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        replacements = {
            ...
            "app.services.market.logos.LOGOS_STORE_DIR": self.market_store_root / "logos",
        }
    ...
    def test_yfinance_search_reuses_shared_verified_session(self) -> None:
        ...
        with patch(
            "app.services.market.logos.get_yfinance_session",
            return_value=shared_session,
        ), patch("app.services.market.logos.yf.Search") as search_mock:
    ...
    def test_known_smh_is_selectable_without_remote_symbol_search(self) -> None:
        with (
            patch("app.services.market.logos.top_used_tickers", return_value=[]),
            patch("app.services.market.logos.build_local_search_items", return_value=[]),
            patch("app.services.market.logos.has_remote_market_access", return_value=False),
            patch("app.services.market.logos.has_profile_record", return_value=False),
            patch("app.services.market.logos.has_logo_asset", return_value=False),
            patch("app.services.market.logos.history_store_path_for") as history_path_mock,
        ):
    ...
    def test_yfinance_ticker_profile_reuses_shared_verified_session(self) -> None:
        ...
        with patch(
            "app.services.market.logos.get_yfinance_session",
            return_value=shared_session,
        ), patch("app.services.market.logos.yf.Ticker") as ticker_mock:
    ...
    def test_unconfigured_search_certificate_failure_logs_ca_guidance(self) -> None:
        ...
        with patch(
            ...
            "app.services.market.logos.ensure_market_store_dir",
        ), patch(
            "app.services.market.logos.top_used_tickers",
            return_value=[],
        ), patch(
            "app.services.market.logos.build_local_alias_search_items",
            return_value=[],
        ), patch(
            "app.services.market.logos.build_local_search_items",
            return_value=[],
        ), patch(
            "app.services.market.logos.load_search_cache_items",
            return_value=[],
        ), patch(
            "app.services.market.logos.has_remote_market_access",
            return_value=True,
        ), patch(
            "app.services.market.logos._search_yfinance_quotes",
            side_effect=ConnectionError(failure),
        ), self.assertLogs("app.services.market.logos", level="WARNING") as captured:
    ...
    def test_local_sk_hynix_suggestion_replaces_symbol_only_profile_name(self) -> None:
        ...
        with patch("app.services.market.logos.has_profile_record", return_value=True), \
                patch("app.services.market.logos.has_logo_asset", return_value=False), \
                patch("app.services.market.logos.load_profile_record", return_value=fallback_record):
    ...
    def test_known_sk_hynix_profile_skips_remote_yfinance_lookup(self) -> None:
        for cached_record in (
            ...
            with self.subTest(cached_record=cached_record):
                with patch("app.services.market.logos.load_profile_record", return_value=cached_record), \
                        patch(
                            "app.services.market.logos.resolve_logo_url_with_fallback",
                            return_value="/market-store/logos/000660.KS.svg",
                        ), \
                        patch("app.services.market.logos._load_yfinance_ticker_info") as info_mock, \
                        patch("app.services.market.logos.has_remote_market_access") as remote_access_mock:
    ...
    def test_cached_named_profile_skips_remote_connectivity_probe(self) -> None:
        ...
        with patch("app.services.market.logos.load_profile_record", return_value=cached_record), \
                patch("app.services.market.logos.resolve_logo_url_with_fallback", return_value="/market-store/logos/000660.KS.svg"), \
                patch("app.services.market.logos.has_remote_market_access") as remote_access_mock:
    ...
    def test_sk_hynix_us_symbols_reuse_korean_primary_listing_logo(self) -> None:
        with create_app().test_request_context():
            with patch("app.services.market.logos.resolve_logo_store_path") as resolve_mock:
    ...
    def test_sk_hynix_alias_logo_bypasses_remote_refresh(self) -> None:
        with patch(
            "app.services.market.logos.resolve_stored_logo_url",
            return_value="/market-store/logos/000660.KS.svg?v=123",
        ), patch("app.services.market.logos.refresh_logo_store") as refresh_mock:
    ...
    def test_known_xqqi_is_selectable_when_remote_discovery_is_unavailable(self) -> None:
        with (
            ...
            patch("app.services.market.logos.has_profile_record", return_value=False),
            patch(
                "app.services.market.logos.has_logo_asset",
    ...
    def test_remote_png_refresh_never_overwrites_an_existing_svg_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("app.services.market.logos.LOGOS_STORE_DIR", logo_dir), \
                    patch("app.services.market.logos.logo_store_path_for", return_value=svg_path), \
                    patch("app.services.market.logos.is_pinned_logo_ticker", return_value=False), \
                    patch("app.services.market.logos.fetch_curated_logo_svg_bytes", return_value=None), \
                    patch("app.services.market.logos.has_remote_market_access", return_value=True), \
                    patch("app.services.market.logos.fetch_remote_logo_bytes", return_value=png_payload):
    ...
    def test_build_quote_profile_payload_uses_known_name_when_yfinance_is_empty(self) -> None:
        with patch("app.services.market.logos._load_yfinance_ticker_info") as info_mock:
    ...
    def test_build_quote_profile_payload_uses_exact_cached_search_name_when_yfinance_is_empty(self) -> None:
        ...
        with patch("app.services.market.logos._load_yfinance_ticker_info", return_value={}), \
                patch(
                    "app.services.market.logos.load_latest_search_cache_item_for_symbol",
    ...
    def test_build_quote_profile_payload_replaces_yfinance_symbol_placeholders(self) -> None:
        ...
        for ticker, expected_name in expected_names.items():
            with self.subTest(ticker=ticker), \
                    patch(
                        "app.services.market.logos._load_yfinance_ticker_info",
    ...
    def test_forced_profile_refresh_preserves_a_cached_name_when_yfinance_returns_a_symbol(self) -> None:
        ...
        with (
            patch("app.services.market.logos.load_profile_record", return_value=record),
            patch("app.services.market.logos.has_remote_market_access", return_value=True),
            patch("app.services.market.logos._load_yfinance_ticker_info", return_value={"longName": "SNDK"}),
            patch("app.services.market.logos.load_latest_search_cache_item_for_symbol", return_value=None),
            patch("app.services.market.logos.upsert_profile_record", return_value=record) as upsert_mock,
            patch("app.services.market.logos.resolve_logo_url_with_fallback", return_value="/market-store/logos/SNDK.svg"),
        ):
    ...
    def test_fetch_quote_profile_keeps_pinned_roundhill_logo_during_forced_refresh(self) -> None:
        ...
        with create_app().test_request_context():
            with patch("app.services.market.logos.load_profile_record", return_value=None), \
                    patch("app.services.market.logos.has_remote_market_access", return_value=True), \
                    patch("app.services.market.logos.yf.Ticker") as ticker_mock, \
                    patch(
                        "app.services.market.logos.upsert_profile_record",
                        return_value={
                            "ticker": "DRAM",
                    ...
                    patch("app.services.market.logos.fetch_remote_logo_bytes", return_value=b"roundhill-logo") as fetch_logo_mock:
    ...
    def test_forced_refresh_does_not_overwrite_roundhill_product_logos(self) -> None:
        with patch("app.services.market.logos.fetch_curated_logo_svg_bytes") as curated_mock, \
                patch("app.services.market.logos.fetch_remote_logo_bytes") as remote_mock:
