...
from app.services.market.date_constraints import nyse_market_session_state
...
from app.services.market.market_freshness import ensure_latest_backtest_intraday_cache
...
class TickerSymbolBoundaryTests(unittest.TestCase):
    ...
    def test_exact_one_minute_history_uses_yahoo_chart_after_yfinance_rate_limit(self) -> None:
        ...
        with (
            patch(
                "app.services.market.market_data._download_daily_history_with_yfinance",
                side_effect=rate_limit_error,
            ),
            patch(
                "app.services.market.market_data._download_one_minute_history_with_yahoo_chart_window",
...
class UsEquitySessionClassificationTests(unittest.TestCase):
    ...
    def test_compare_overnight_switch_requires_true_overnight_provider(self) -> None:
        with (
            patch("app.services.market.market_data._load_compare_overnight_market_settings", return_value=None),
            patch("app.services.market.market_data.yf.download", create=True),
        ):
    ...
    def test_compare_overnight_falls_back_to_yfinance_after_longbridge_failure(self) -> None:
        ...
        with (
            patch("app.services.market.market_data._load_compare_overnight_market_settings", return_value=settings),
            patch(
                "app.services.market.market_data.fetch_longbridge_compare_one_day_history",
                side_effect=ConnectionError("Longbridge is unavailable."),
            ),
            patch(
                "app.services.market.market_data._fetch_yfinance_compare_one_day_extended_history",
...
class MarketDataFreshnessTests(unittest.TestCase):
    ...
    def test_local_one_minute_loader_normalizes_in_memory_without_writing(self) -> None:
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with (
                patch(
                    "app.services.market.market_data.market_ticker_store_aliases",
                    return_value=("NVDA",),
                ),
                patch(
                    "app.services.market.market_data.history_store_path_for_interval",
                    return_value=path,
                ),
                patch("app.services.market.market_data.write_parquet_atomic") as write_mock,
            ):
    ...
    def test_intraday_only_backtest_refresh_reports_failure_without_daily_access(self) -> None:
        with (
            patch(
                "app.services.market.market_freshness.is_one_minute_store_fresh",
                return_value=False,
            ),
            patch(
                "app.services.market.market_freshness.refresh_one_minute_store",
                side_effect=ValueError("rate limited"),
            ) as refresh_mock,
            patch(
                "app.services.market.market_freshness.ensure_fresh_history_store",
    ...
    def test_remote_disable_blocks_direct_yahoo_fallback(self) -> None:
        with (
            patch("app.services.market.market_data.yf.download") as primary,
            patch("app.services.market.market_data.download_yahoo_chart_daily_history") as fallback,
            patch("app.services.market.market_data._load_longbridge_market_settings") as broker,
        ):
    ...
    def test_remote_market_access_environment_override_blocks_minute_refresh(self) -> None:
        with (
            patch.dict("os.environ", {"WORTHWARD_REMOTE_MARKET_ACCESS": "disabled"}),
            patch(
                "app.services.market.market_data._download_recent_one_minute_history_with_yfinance"
            ) as yfinance_download,
            patch("app.services.market.market_data._load_longbridge_market_settings") as longbridge_settings,
            patch("app.services.market.market_data.yf.download") as yfinance_transport,
        ):
            with self.assertRaisesRegex(YfinanceDownloadError, "Remote market access is disabled"):
    ...
    def test_refresh_history_store_skips_write_when_remote_has_no_newer_trading_day(self) -> None:
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with (
                patch(
                    "app.services.market.market_data.history_store_path_for",
                    return_value=path,
                ),
                patch(
                    "app.services.market.market_data.has_remote_market_access",
                    return_value=True,
                ),
                patch(
                    "app.services.market.market_data._download_daily_history_with_yfinance",
    ...
    def test_fetch_history_repairs_missing_dividend_actions_from_adjustment_steps(self) -> None:
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with (
                patch("app.services.market.market_data.market_ticker_store_aliases", return_value=["MSFT"]),
                patch("app.services.market.market_data.history_store_path_for_interval", return_value=history_path),
                patch("app.services.market.market_data.refresh_history_store", side_effect=repair_store) as refresh_mock,
            ):
    ...
    def test_fetch_history_repairs_legacy_cache_missing_split_actions(self) -> None:
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with (
                patch("app.services.market.market_data.market_ticker_store_aliases", return_value=["NVDA"]),
                patch("app.services.market.market_data.history_store_path_for_interval", return_value=history_path),
                patch("app.services.market.market_data.refresh_history_store", side_effect=repair_store) as refresh_mock,
            ):
    ...
    def test_fetch_history_marks_legacy_split_actions_unverified_after_refresh_failure(self) -> None:
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with (
                ...
                patch("app.services.market.market_data.history_store_path_for_interval", return_value=history_path),
                patch(
                    "app.services.market.market_data.refresh_history_store",
                    side_effect=ValueError("rate limited"),
                ),
            ):
                result = fetch_history("NVDA", False, dividend_mode="price")
    ...
    def test_fetch_history_keeps_zero_dividend_cache_when_adjustment_ratio_is_stable(self) -> None:
        ...
        with TemporaryDirectory() as tempdir:
            ...
            with (
                patch("app.services.market.market_data.market_ticker_store_aliases", return_value=["ADBE"]),
                patch("app.services.market.market_data.history_store_path_for_interval", return_value=history_path),
                patch("app.services.market.market_data.refresh_history_store") as refresh_mock,
            ):
    ...
    def test_download_full_history_serializes_concurrent_yfinance_requests(self) -> None:
        ...
        with patch("app.services.market.market_data.yf.download", side_effect=fake_download):
    ...
    def test_yfinance_fallback_download_receives_shared_verified_session(self) -> None:
        ...
        with (
            patch(
                "app.services.market.market_data.get_yfinance_session",
                return_value=shared_session,
            ),
            patch(
                "app.services.market.market_data.yf.download",
    ...
    def test_daily_history_uses_direct_yahoo_chart_before_optional_longbridge(self) -> None:
        ...
        with (
            ...
            patch(
                "app.services.market.market_data.download_yahoo_chart_daily_history",
                return_value=chart_history,
            ) as chart_mock,
            patch("app.services.market.market_data._load_longbridge_market_settings", return_value=None),
            patch("app.services.market.market_data._download_daily_history_with_longbridge") as longbridge_mock,
        ):
    ...
    def test_download_daily_history_with_fallback_steps_down_from_max_for_newly_listed_tickers(self) -> None:
        ...
        with (
            patch("app.services.market.market_data._load_longbridge_market_settings", return_value=None),
            patch("app.services.market.market_data.yf.download", side_effect=fake_download) as download_mock,
        ):
    ...
    def test_download_full_history_canonicalizes_share_class_symbol_for_yfinance(self) -> None:
        ...
        with patch("app.services.market.market_data.yf.download", return_value=fake_history) as download_mock:
    ...
    def test_download_full_intraday_history_uses_yfinance_without_longbridge(self) -> None:
        ...
        with (
            patch("app.services.market.market_data._load_longbridge_market_settings", return_value=None),
            patch("app.services.market.market_data._download_one_minute_history_with_longbridge") as longbridge_mock,
            patch(
                "app.services.market.market_data._download_recent_one_minute_history_with_yfinance",
    ...
    def test_download_full_intraday_history_uses_yfinance_before_configured_longbridge(self) -> None:
        ...
        with (
            patch("app.services.market.market_data._load_longbridge_market_settings", return_value=object()),
            patch("app.services.market.market_data._download_one_minute_history_with_longbridge") as longbridge_mock,
            patch(
                "app.services.market.market_data._download_recent_one_minute_history_with_yfinance",
                return_value=yfinance_history,
    ...
    def test_download_full_intraday_history_uses_longbridge_only_after_yfinance_fails(self) -> None:
        ...
        with (
            ...
            patch(
                "app.services.market.market_data._download_one_minute_history_with_longbridge",
    ...
    def test_intraday_history_skips_longbridge_for_unsupported_korean_ticker(self) -> None:
        ...
        with (
            ...
            patch("app.services.market.market_data._load_longbridge_market_settings") as settings_mock,
            patch("app.services.market.market_data._download_one_minute_history_with_longbridge") as longbridge_mock,
        ):
        ...
        with (
            patch("app.services.market.market_data.has_recent_one_minute_store", return_value=True),
            patch(
                "app.services.market.market_data._download_recent_one_minute_history_with_yfinance",
                side_effect=rate_limit_error,
            ) as refresh_yfinance_mock,
            patch("app.services.market.market_data._load_longbridge_market_settings") as refresh_settings_mock,
            patch("app.services.market.market_data.refresh_longbridge_one_minute_store") as refresh_longbridge_mock,
        ):
    ...
    def test_refresh_one_minute_store_uses_yfinance_before_configured_longbridge(self) -> None:
        ...
        with (
            patch("app.services.market.market_data.has_recent_one_minute_store", return_value=False),
            patch("app.services.market.market_data._load_longbridge_market_settings", return_value=object()),
            patch(
                "app.services.market.market_data._download_recent_one_minute_history_with_yfinance",
                return_value=yfinance_history,
            ) as yfinance_mock,
            patch("app.services.market.market_data._upsert_one_minute_store", return_value=expected_path),
            patch("app.services.market.market_data.refresh_longbridge_one_minute_store") as longbridge_mock,
        ):
    ...
    def test_explicit_longbridge_minute_refresh_bypasses_yahoo(self) -> None:
        ...
        with (
            patch("app.services.market.market_data.is_remote_market_access_disabled", return_value=False),
            patch("app.services.market.market_data._supports_longbridge_history_fallback", return_value=True),
            patch("app.services.market.market_data._load_longbridge_market_settings", return_value=settings),
            patch(
                "app.services.market.market_data.intraday_history_store_path_for",
            ...
            patch(
                "app.services.market.market_data.refresh_longbridge_one_minute_store"
            ) as longbridge_mock,
        ):
    ...
    def test_realtime_quote_batch_rotates_one_individual_recovery_after_batch_failure(self) -> None:
        ...
        with (
            patch("app.services.market.market_data._yfinance_realtime_recovery_cursor", 0),
            patch(
                "app.services.market.market_data._download_daily_history_with_yfinance",
                side_effect=fake_download,
            ) as download_mock,
            patch("app.services.market.market_data.LOGGER.warning") as warning_mock,
        ):
    ...
    def test_realtime_quote_batch_rate_limit_skips_individual_retries_and_cools_down(self) -> None:
        ...
        with (
            patch("app.services.market.market_data._yfinance_rate_limit_until", 0.0),
            patch("app.services.market.market_data._yfinance_rate_limit_failures", 0),
            patch(
                "app.services.market.market_data._download_daily_history_with_yfinance",
                side_effect=rate_limit_error,
            ) as download_mock,
    ...
    def test_yfinance_rate_limit_backoff_doubles_and_is_bounded(self) -> None:
        with (
            patch("app.services.market.market_data._yfinance_rate_limit_until", 0.0),
            patch("app.services.market.market_data._yfinance_rate_limit_failures", 0),
            patch("app.services.market.market_data.monotonic", side_effect=[100.0, 400.0, 1_000.0, 2_200.0]),
        ):
    ...
    def test_yfinance_rate_limit_cooldown_blocks_the_transport_before_another_request(self) -> None:
        with (
            patch("app.services.market.market_data._yfinance_rate_limit_until", 160.0),
            patch("app.services.market.market_data.monotonic", return_value=100.0),
            patch("app.services.market.market_data.yf.download") as download_mock,
        ):
    ...
    def test_longbridge_realtime_quotes_select_pre_market_price(self) -> None:
        ...
        with (
            patch("app.services.market.market_data.load_broker_settings", return_value=settings),
            patch(
                "app.services.market.market_data.nyse_market_session_state",
                return_value={"session": "pre", "session_date": "2026-07-20"},
            ),
            patch(
                "app.services.market.market_data.run_longbridge_cli_json",
                return_value=[{
                    "symbol": "DRAM.US",
                    "last": "52.72",
    ...
    def test_longbridge_realtime_quotes_skip_transport_outside_supported_scope(self) -> None:
        ...
        with (
            ...
            patch("app.services.market.market_data.run_longbridge_cli_json") as cli_json,
        ):
        ...
        with (
            ...
            patch("app.services.market.market_data.run_longbridge_cli_json") as non_us_cli_json,
        ):
    ...
    def test_longbridge_realtime_quotes_support_legacy_api_key_configuration(self) -> None:
        ...
        with (
            ...
            patch(
                "app.services.market.market_data.get_longbridge_quote_context",
    ...
    def test_intraday_endpoint_uses_market_fallback_for_a_missing_active_day(self) -> None:
        prior_day = pd.date_range(
            "2026-08-10 09:30",
            periods=390,
            freq="min",
        ).strftime("%Y-%m-%d %H:%M").tolist()
        prior_dataset = ohlc_frame_for_dates("DRAM", prior_day)
        current_dataset = ohlc_frame_for_dates(
            "DRAM",
            [*prior_day, "2026-08-11 09:30"],
        )
        ...
        self.assertEqual(response.get_json()["count"], 391)
    ...
    def test_intraday_endpoint_refreshes_a_partial_completed_trading_day(self) -> None:
        partial_day = pd.date_range(
            "2026-08-11 09:30",
            periods=92,
            freq="min",
        ).strftime("%Y-%m-%d %H:%M").tolist()
        dataset = ohlc_frame_for_dates("DRAM", partial_day)
        refreshed_dataset = ohlc_frame_for_dates(
            "DRAM",
            pd.date_range(
                "2026-08-11 09:30",
                periods=390,
        ...
        with TemporaryDirectory() as tempdir:
            ...
            def refresh_with_complete_history(_ticker, *, days):
                self.assertEqual(days, 30)
                refreshed_dataset.to_parquet(store_path, index=False)
                return SimpleNamespace(source="yfinance_30d")
            ...
            with (
                ...
                patch(
                    "app.web.runtime.refresh_recent_one_minute_store_with_yfinance",
                    side_effect=refresh_with_complete_history,
                ) as refresh_mock,
                patch(
                    "app.web.runtime.nyse_market_session_state",
                    return_value={
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["count"], 390)
        self.assertEqual(response.get_json()["rows"][-1]["date"], "2026-08-11 15:59")
        refresh_mock.assert_called_once_with("DRAM", days=30)

    def test_intraday_endpoint_uses_longbridge_when_yahoo_cannot_fill_a_completed_day(self) -> None:
        partial_day = pd.date_range(
            "2026-08-11 09:30",
            periods=92,
            freq="min",
        ).strftime("%Y-%m-%d %H:%M").tolist()
        completed_day = pd.date_range(
            "2026-08-11 09:30",
            periods=390,
            freq="min",
        ).strftime("%Y-%m-%d %H:%M").tolist()
        partial_dataset = ohlc_frame_for_dates("DRAM", partial_day)
        completed_dataset = ohlc_frame_for_dates("DRAM", completed_day)

        with TemporaryDirectory() as tempdir:
            store_path = Path(tempdir) / "DRAM_1m.parquet"
            partial_dataset.to_parquet(store_path, index=False)

            def refresh_with_longbridge(_ticker):
                completed_dataset.to_parquet(store_path, index=False)
                return SimpleNamespace(source="longbridge_fallback")
            ...
            with (
                ...
                patch(
                    "app.web.runtime.refresh_recent_one_minute_store_with_yfinance",
                    side_effect=YfinanceDownloadError("Yahoo is rate limited"),
                ) as yahoo_refresh_mock,
                patch(
                    "app.web.runtime.refresh_one_minute_store_with_longbridge",
                    side_effect=refresh_with_longbridge,
                ) as fallback_refresh_mock,
                patch(
                    "app.web.runtime.nyse_market_session_state",
                    return_value={
    ...
    def test_ensure_fresh_history_store_refreshes_stale_daily_cache(self) -> None:
        with (
            patch("app.services.market.market_data.is_daily_store_fresh", return_value=False),
            patch("app.services.market.market_data.has_remote_market_access", return_value=True),
            patch("app.services.market.market_data.refresh_history_store") as refresh_mock,
        ):
    ...
    def test_ensure_fresh_history_store_uses_longbridge_when_yahoo_is_unavailable(self) -> None:
        ...
        with (
            patch("app.services.market.market_data.is_daily_store_fresh", return_value=False),
            patch("app.services.market.market_data.has_remote_market_access", return_value=False),
            patch("app.services.market.market_data._load_longbridge_market_settings", return_value=longbridge_settings),
            patch("app.services.market.market_data.refresh_history_store") as refresh_mock,
        ):
