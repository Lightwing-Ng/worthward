...
from app.services import investment_import_compat as compat


PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMPAT_SOURCE = PROJECT_ROOT / "app/services/investment_import_compat.py"
SEAM_CALLERS = (
...
def test_domain_modules_reach_seams_only_through_the_declared_bridge() -> None:
    ...
    for path in sorted((PROJECT_ROOT / "app/services").glob("investment_import_*.py")):
        if path.name == "investment_import_compat.py":
            continue
